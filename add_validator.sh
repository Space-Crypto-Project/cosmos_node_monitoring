...
yq -i -y '.scrape_configs[] |= (.job_name as $name | .static_configs += if $name == "prometheus" then [] else ([{targets:["'$1'" + (if $name == "node" then ":9100" elif $name == "cosmos" then ":'$2'" end)], labels:(if $name == "node" then {instance: "'$5'"} elif $name == "cosmos" then {custom_instance: "'$5'"} else {} end)}]) end)' ./prometheus/prometheus.yml
...
#   yq -i '.scrape_configs[] |= (if .job_name == "node" then .static_configs += [{"targets": ["'$1':9100"], "labels": {"instance": "'$5'"}}] else . end)' ./prometheus/prometheus.yml
#   echo "Target $1:9100 added under 'node' job."
# else
#   # Print a message if the target exists
